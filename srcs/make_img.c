...
/*   Updated: 2020/06/17 18:39:52 by hthomas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_draw_start_end(t_dda *dda, t_scene *s)
{
	...
	if (dda->side % 2 == 0)
		dda->perpWallDist = (dda->coord.h - s->pos.x +\
		(1 - dda->step.h) / 2.) / dda->rayDir.x;
	else
		dda->perpWallDist = (dda->coord.w - s->pos.y +\
		(1 - dda->step.w) / 2.) / dda->rayDir.y;
	dda->lineHeight = (s->res.h / dda->perpWallDist);
