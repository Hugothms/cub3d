...
/*   Updated: 2020/06/23 14:57:54 by hthomas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	do_sprite(t_img *img, t_dda *dda, t_scene *s)
{
	...
	for (int i = 0; i < dda->index_sprite; i++)
	{
		...
		//transform sprite wits->res.h the inverse camera matrix
		...
		double transformY = invDet * (-s->plane.y * spriteX + s->plane.x * spriteY); //this is actually the depts->res.h inside the screen, that what Z is in 3D

		int spriteScreenX = (int)((s->res.w / 2) * (1 + transformX / transformY));

		//calculate height of the sprite on screen
		int spriteHeight = abs((int)(s->res.h / (transformY))); //using 'transformY' instead of the real distance prevents fisheye
		...
		if (drawEndY >= s->res.h)
			drawEndY = s->res.h - 1;
		//calculate widts->res.h of the sprite
		...
		//loop througs->res.h every vertical stripe of the sprite on screen
		for (int stripe = drawStartX; stripe < drawEndX; stripe++)
		{
			...
			//4) dda->perpWallDist, wits->res.h perpendicular distance
			if (transformY >= 0 && stripe > 0 && stripe < s->res.w && transformY < dda->perpWallDist[stripe])
			{
				for (int y = drawStartY; y < drawEndY; y++) //for every pixel of the current stripe
				{
					int d = (y)*256 - s->res.h * 128 + spriteHeight * 128; //256 and 128 factors to avoid floats
